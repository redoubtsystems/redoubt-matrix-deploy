...
def _send_invite_email(to_email, username, invite_url):
...
def create_user():
    ...
    if data.get("invite_mode") == "email":
        ...
        invite_url = f"{ADMIN_URL}/invite/{token}"
        try:
            _send_invite_email(email, username, invite_url)
            flash(f"User created. Invite email sent to {email}.", "success")
...
def resend_invite(username):
    ...
    try:
        _send_invite_email(email, username, invite_url)
        flash(f"Password reset and invite email sent to {email}.", "success")
    except Exception as e:
...
## Invite routes (public — no login required)
@app.route("/invite/<token>", methods=["GET"])
def invite_get(token):
    inv = _INVITES.get(token)
    if not inv or datetime.now(timezone.utc) > inv["expires"]:
        return render_template(
            "error.html",
            title="Link Expired",
            message="This invite link has expired or is invalid. Please contact your administrator.",
            status=404,
        ), 404
    return render_template("userInvite.html", token=token, username=inv["username"])


@app.route("/invite/<token>", methods=["POST"])
def invite_post(token):
    inv = _INVITES.get(token)
    if not inv or datetime.now(timezone.utc) > inv["expires"]:
        return render_template(
            "error.html",
    ...
    user_id = f"@{inv['username']}:{SERVER_NAME}"

    # Authenticate as the new user using the server-generated initial password
    try:
        r = requests.post(
            f"{SYNAPSE_URL}/_matrix/client/v3/login",
            json={
        ...
        flash("Cannot reach the server. Please try again.", "error")
        return render_template("userInvite.html", token=token, username=inv["username"])

    if not r.ok:
        flash("Account setup failed. Please contact your administrator.", "error")
        return render_template("userInvite.html", token=token, username=inv["username"])

    access_token = r.json()["access_token"]

    # Change to the user's chosen password and close out the temp session
    try:
        requests.post(
            f"{SYNAPSE_URL}/_matrix/client/v3/account/password",
        ...
        pass  # Password likely changed; log out best-effort
...
def get_rooms():
    ...
    return render_template("rooms.html", rooms=rooms, total=data.get("total_rooms", 0))
...
def subscription():
    ...
    plan_display = _PLAN_DISPLAY.get(
        (data or {}).get("plan_type", ""), (data or {}).get("plan_type", "")
    )
